 ...
 */
const createParser = (compilation) => {
  ...
  return ({ name = '', chunks }) => ({
    name,
    files: chunks.flatMap(({ files }) =>
      [...files].map((fileName) => `${filePrefix}${fileName}`),
    ),
 ...
 */
module.exports.parseEntries = (compilation) =>
  [...compilation.entrypoints.values()].map(createParser(compilation));
